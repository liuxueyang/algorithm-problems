...
public:
  vector<int> twoSum(vector<int>& nums, int target) {
    ...
    sort(vp.begin(), vp.end(), less<PII>());

    for (auto i = 0; i < vp.size(); ++i) {
      auto it = lower_bound(vp.begin() + i + 1, vp.end(),
			    make_pair<int, int>(target - vp[i].first, 0));

      if (it != vp.end() && it->first == target - vp[i].first)
	return vector<int> {vp[i].second, it->second};
    }
