...
public:
  int lengthOfLongestSubstring(string s) {
    VI found(257, -1);
    int result { 0 }, last { -1 }, i { 0 };

    while (i < s.size()) {
      if (-1 != found[s[i]]) last = max_(found[s[i]], last);
      found[s[i]] = i;
      result = max_(result, i - last);
      i++;
    }
