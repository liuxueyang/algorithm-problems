...
public:
  vector<vector<int>> subsets(vector<int>& nums) {
    ...
    for (int i = 0; i < n; ++i)
      result.push_back(bin_to_vi(nums, i));

    return result;
  }

  VI bin_to_vi(VI &nums, int n) {
    VI result;
    int i = 0;

    while (n) {
      if (n & 1) result.push_back(nums[i]);
      i++;
      n >>= 1;
    }
...
public:
  vector<vector<int>> subsets(vector<int>& nums) {
    ...
    helper(result, VI {}, 0, nums);

    return result;
  }

  void helper(VVI &result, VI tmp, int start, VI &nums) {
